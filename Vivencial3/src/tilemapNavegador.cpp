...
int cursor_i = 0; // posição atual do cursor (linha)
int cursor_j = 0; // posição atual do cursor (coluna)
...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Tilemap - Navegaçao", nullptr, nullptr);
	if (!window)
	{
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
    ...
    {
        if (key == GLFW_KEY_UP)          // Norte
            cursor_i = std::max(cursor_i - 1, 0);
        else if (key == GLFW_KEY_DOWN)   // Sul
            cursor_i = std::min(cursor_i + 1, TILEMAP_HEIGHT - 1);
        else if (key == GLFW_KEY_LEFT)   // Oeste
            cursor_j = std::max(cursor_j - 1, 0);
        else if (key == GLFW_KEY_RIGHT)  // Leste
            cursor_j = std::min(cursor_j + 1, TILEMAP_WIDTH - 1);
        else if (key == GLFW_KEY_Q)      // NW
        {
            cursor_i = std::max(cursor_i - 1, 0);
            cursor_j = std::max(cursor_j - 1, 0);
        }
...
Tile cursor_tile = tileset[6]; // exemplo: tile rosa é o tile 5

// Garante que o tile rosa tenha o mesmo tamanho que o tile do mapa
cursor_tile.dimensions = tileset[map[cursor_i][cursor_j]].dimensions;

float x = x0 + (cursor_j - cursor_i) * cursor_tile.dimensions.x / 2.0;
float y = y0 + (cursor_j + cursor_i) * cursor_tile.dimensions.y / 2.0;

mat4 model = mat4(1);
model = translate(model, vec3(x, y, 0.0));
model = scale(model, cursor_tile.dimensions);
glUniformMatrix4fv(glGetUniformLocation(shaderID, "model"), 1, GL_FALSE, value_ptr(model));

vec2 offsetTex;
offsetTex.s = cursor_tile.iTile * cursor_tile.ds;
offsetTex.t = 0.0;
glUniform2f(glGetUniformLocation(shaderID, "offsetTex"), offsetTex.s, offsetTex.t);

glBindVertexArray(cursor_tile.VAO);
glBindTexture(GL_TEXTURE_2D, cursor_tile.texID);
glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
}
